...
## Sistema de Clientes (Spanish local-business subtree)

A **second, separate offering** lives under `app/sistema-de-clientes/*`: a Spanish (`es-MX`) lead-gen funnel for local Guadalajara service businesses (website + WhatsApp AI agent + Google Business Profile). It is **deliberately isolated** from the English AI consultancy site — different audience, different copy, **not linked from the main nav** (but in the sitemap). Full spec: `docs/clientes.md`; build plan: `docs/sistema-de-clientes-plan.md`.
...
- **Content is data-driven** like the main site: `content/sistema-de-clientes/verticales.ts` (per-vertical objects, `published` flag — only 3 of 8 are live: paneles-solares, impermeabilizacion, portones-electricos) and `content/sistema-de-clientes/hub.ts` (shared copy + the `SistemaContent` adapter both hub and verticals render through `_components/SistemaTemplate`).
- **Config** lives in `lib/site.ts` as the `SC` object + `waLink()` helper. Envs: `NEXT_PUBLIC_SC_WHATSAPP`, `NEXT_PUBLIC_SC_BOOKING_URL` (both in `turbo.json` globalEnv). Same graceful-degradation house style — missing WhatsApp/booking config makes CTAs fall back to email, never a broken link.
...
- **Styling:** reuses the existing design tokens with a `.sc-*` layer in `globals.css` (slightly warmer, WhatsApp-green CTAs). Interactive islands only: `ScRoiCalc`, `ScFaq`, `ScWhatsAppButton`/`StickyWhatsApp`, `ScBookingButton`, `ScLeadForm`.
